    ...
    useEffect(() => {
        ...
        if (isReadOnlyMode && (prefilledData as any).items && Array.isArray((prefilledData as any).items)) {
            ...
            const explicitAdvance = itemsList.find((i: any) => {
                if (!(i.is_advance || i.reference_type === 'ADVANCE')) return false;
                const ref = i.advance_ref_no || i.reference_number || i.reference_id || i.ref_no || '';
                return ref && ref !== vNum && ref !== 'ADVANCE';
            });

            const advanceItem = explicitAdvance || itemsList.find((i: any) => i.is_advance || i.reference_type === 'ADVANCE');

            if (advanceItem) {
                const itemRefNo = advanceItem.advance_ref_no || advanceItem.reference_number || advanceItem.reference_id || advanceItem.ref_no || (prefilledData as any).reference_number || '';
                const isAutoGenerated = itemRefNo && vNum && (itemRefNo === vNum || itemRefNo === 'ADVANCE');

                // Show if it's NOT auto-generated, OR if we explicitly found an explicit advance
                if (!isAutoGenerated || explicitAdvance) {
    ...
    useEffect(() => {
        if (selectedReceiptConfig && receiptVoucherConfigs.length > 0) {
            ...
            if (config) {
                ...
                if (prefilledData && ((prefilledData as any).receiptVoucherNumber || (prefilledData as any).invoiceNumber)) {
                    setVoucherNumber((prefilledData as any).receiptVoucherNumber || (prefilledData as any).invoiceNumber);
                } else if (!isReadOnlyMode && config.enable_auto_numbering) {
        ...
        try {
            ...
            if (editingVoucherId) {
                payload.items = [{
                    ...
                    advance_ref_no: voucherNumber || 'ADVANCE',
            ...
            if (response && response.next_voucher_number) {
