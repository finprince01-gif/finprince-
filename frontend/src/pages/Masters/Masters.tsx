...
const MastersPage: React.FC<MastersPageProps> = ({
   ...
   */
  const getCurrentFYSuffix = (): string => {
    const now = new Date();
  ...
  const getDefaultVoucherConfig = (voucherType: string) => {
  ...
  // Fetch existing voucher configurations; auto-create default if none exist
  const fetchVoucherConfigurations = async () => {
    try {
      ...
      const list = configs || [];
      setExistingVouchers(list);

      // Auto-create default if the canonical default name is absent for this tab
      const def = getDefaultVoucherConfig(selectedVoucher);
      const alreadyHasDefault = list.some(
        (v: any) => v.voucher_name === def.voucher_name
      );
      if (!alreadyHasDefault) {
        ...
        try {
          ...
          setExistingVouchers(refreshed || []);
        } catch {
          // Ignore if already created by another session
        }
      }
    } catch (error) {
