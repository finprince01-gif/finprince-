...
export const InventoryCategoryWizard: React.FC<InventoryCategoryWizardProps> = ({
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // 2. Update Group (when showSubgroup is false and editing)
            else if (!showSubgroup && selectedNode.data.id && onEditCategory && selectedNode.level === 1 && isEditing) {
            ...
            // 3. Update Subgroup (when showSubgroup is true and editing)
            else if (showSubgroup && selectedNode.data.id && onEditCategory) {
                await onEditCategory({
                    id: selectedNode.data.id,
                    category: selectedNode.data.category,
                    group: selectedNode.data.group,
                        ...
                        <form onSubmit={handleSubmit} className="space-y-5">
                            ...
                            <div className="pt-4">
                                <button
                                    ...
                                    className={`w-full py-2.5 px-4 rounded font-medium shadow-none border border-slate-200-none border border-slate-200 transition-all focus:outline-none focus:ring-2 focus:ring-offset-2 text-sm ${selectedNode && (showSubgroup ? formData.subgroup.trim() : formData.group.trim())
                                ...
                                >
                                    {showSubgroup ? 'Create Subgroup' : (isEditing ? 'Update Group' : 'Create Group')}
                                </button>
                            </div>
                        </form>
                    </div>
