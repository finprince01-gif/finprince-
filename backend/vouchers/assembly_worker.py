...
class AssemblyWorker(BaseWorker):
    ...
    async def handle_task(self, task: Dict[str, Any]):
        ...
        state = orchestrator.get_barrier_state(record_id, expected_pages)
        if not state:
        ...
        if not state["is_ready"] and duration > timeout_seconds:
            ...
            state = orchestrator.get_barrier_state(record_id, expected_pages) or state
            state["is_ready"] = True
        ...
        if not state["is_ready"]:
            ...
            orchestrator.update_session_status(record_id, "PROCESSING", progress=progress)
        ...
        if not is_failed_assembly:
            ...
            orchestrator.update_session_status(record_id, "FAILED", progress=100.0)
