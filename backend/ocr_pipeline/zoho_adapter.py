...
class ZohoAdapter:
    ...
    def resolve_zoho_row(self, invoice: Dict[str, Any], item: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # [ZOHO_GROUP_KEY] 
        logger.info(f"[ZOHO_GROUP_KEY] gstin={gstin} inv_no={invoice.get('invoice_no')}")
        ...
        logger.info(f"[ZOHO_EXPORT_ROW] inv={inv_no} item='{item.get('description')[:20]}...' row_keys={list(row.keys())}")
        return row
        ...
        for inv in invoices:
            ...
            for itm in reconstructed:
                normalized_items.append({
                    "description": itm.get("description") or itm.get("item_name") or "",
                    "hsn_sac": itm.get("hsn_sac") or itm.get("hsn_code") or "",
                    "qty": self._to_float(itm.get("qty") or itm.get("quantity")),
                    "uom": itm.get("uom") or "",
                    "rate": self._to_float(itm.get("rate") or itm.get("item_rate")),
                    "taxable_value": self._to_float(itm.get("taxable_value")),
                    "igst": self._to_float(itm.get("igst") or itm.get("igst_amount")),
                    "cgst": self._to_float(itm.get("cgst") or itm.get("cgst_amount")),
                    "sgst": self._to_float(itm.get("sgst") or itm.get("sgst_amount")),
                    "invoice_value": self._to_float(itm.get("invoice_value") or itm.get("amount")),
                })
            ...
            canonical = {
                "invoice_no": inv.get("invoice_no") or inv.get("invoice_number") or "",
                "invoice_date": inv.get("invoice_date") or "",
                "vendor_name": inv.get("vendor_name") or "",
                "gstin": gstin,
                "branch": inv.get("branch") or "",
                "bill_address_from": inv.get("bill_address_from") or inv.get("bill_from") or inv.get("vendor_address") or "",
                "bill_address_to": inv.get("bill_address_to") or inv.get("billing_address") or "",
                "place_of_supply": pos,
                "total_taxable_value": self._to_float(inv.get("total_taxable_value")),
                "total_igst": self._to_float(inv.get("total_igst")),
                "total_cgst": self._to_float(inv.get("total_cgst")),
                "total_sgst": self._to_float(inv.get("total_sgst")),
                "invoice_total": self._to_float(inv.get("invoice_total") or inv.get("total_invoice_value")),
                "irn": inv.get("irn") or "",
            ...
            logger.info(f"[ZOHO_CANONICAL_OUTPUT] inv={canonical['invoice_no']} total={canonical['invoice_total']} items={len(normalized_items)}")
    ...
    def transform(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for inv in final_invoices:
            ...
            # [RULE #7] HARD REJECT MALFORMED ROWS
        ...
        if zoho_rows:
            logger.info(f"[ZOHO_FIRST_ROW_KEYS] keys={list(zoho_rows[0].keys())}")
            print(f"[ZOHO_DIAGNOSTICS] rows={len(zoho_rows)} first_row_inv={zoho_rows[0].get('Invoice No')}")
