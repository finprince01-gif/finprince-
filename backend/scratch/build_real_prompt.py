...
# Read page 1 OCR text
with open("page1_ocr.txt", "r", encoding="utf-8") as f:
    page_ocr_text = f.read().strip()

# Clean up OCR text (as done in extraction.py line 971-972)
import re
page_ocr_text = re.sub(r'[ \t]+', ' ', page_ocr_text).strip()
page_ocr_text = re.sub(r'(\r\n|\r|\n){2,}', '\n\n', page_ocr_text)

page_isolated_prompt = f"{base_prompt}\n\n### [PAGE 1 OCR DATA]\n{page_ocr_text}"

print(f"Computed Isolated Prompt Length: {len(page_isolated_prompt)} characters")

# Save to REAL_QWEN_PROMPT.txt
out_path = r"c:\108\AI-accounting-0.03\backend\REAL_QWEN_PROMPT.txt"
with open(out_path, "w", encoding="utf-8") as f:
    f.write(page_isolated_prompt)

print(f"Saved real prompt to: {out_path}")
