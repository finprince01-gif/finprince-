...
# I need to separate `ledgerViewMode === 'ledger'` and `ledgerViewMode === 'journal'`.
# Currently they are combined as `(ledgerViewMode === 'ledger' || ledgerViewMode === 'journal') && (`

start_marker = "{/* ── LEDGER / JOURNAL VIEW ── */}"
                ...
                <table className="erp-table min-w-full">
                  <thead className="bg-[#F8F9FA] border-b border-slate-200">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-right text-xs font-bold text-gray-500 uppercase tracking-wider border-r border-slate-200">Running Bal</th>
                  ...
                  <tbody className="bg-white divide-y divide-gray-100">
                    {filteredDrillData.length > 0 ? filteredDrillData.map((e, idx) => (
                      <tr key={`dd-${idx}`}
                        ...
                        <td className={`px-4 py-3 text-sm font-mono text-right font-bold whitespace-nowrap border-r border-gray-50 ${e.balanceType==='Dr'?'text-orange-600':e.balanceType==='Cr'?'text-green-700':'text-gray-400'}`}>
                        ...
                        <td className="px-4 py-3 text-center">
                          {e.voucherType !== 'Opening' && (
                            <button onClick={() => setSelectedTransaction({ ...e, ledgerName: drillDownLedger })} className="text-indigo-600 hover:text-indigo-900 mx-auto" title="View Transaction">
                      ...
                      <React.Fragment key={`dd-j-${idx}`}>
                        <tr className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                          ...
                          <td className="px-6 py-4 text-sm text-right font-bold text-gray-900 border-r border-gray-50">
                            {e.balance>0 ? <>{`₹${e.balance.toFixed(2)} `}<span className={`text-[10px] font-normal uppercase ${e.balanceType==='Dr'?'text-orange-600':'text-green-700'}`}>{e.balanceType}</span></> : <span className="text-gray-300">₹0.00</span>}
                          </td>
                          <td className="px-6 py-4 text-sm font-bold text-indigo-600 text-right border-r border-gray-50">{e.debit>0?`₹${e.debit.toFixed(2)}`:''}</td>
                          <td className="px-6 py-4 text-sm font-bold text-gray-900 text-right border-r border-gray-50">{e.credit>0?`₹${e.credit.toFixed(2)}`:''}</td>
                            ...
                            <td className="px-6 py-1.5 border-r border-gray-50 pl-12">
                              <div className="flex justify-between items-center w-full text-xs font-medium text-gray-700">
                                ...
                                <div className="flex items-center gap-1">
                                  <span className="font-bold">₹{e.debit.toFixed(2)}</span>
                          ...
                          <tr className="bg-white border-b border-gray-50/30">
                            ...
                            <td className="px-6 py-1.5 border-r border-gray-50 pl-20">
                              <div className="flex justify-between items-center w-full text-xs font-bold text-indigo-600">
                                <span>{drillDownLedger} A/c</span>
                                <div className="flex items-center gap-1">
                                  <span className="font-bold">₹{e.credit.toFixed(2)}</span>
                  ...
                  {filteredDrillData.length > 0 && (
                    <tfoot className="bg-gray-50 border-t-2 border-gray-200 font-bold">
                      <tr>
                        ...
                        <td className={`px-6 py-4 text-sm font-mono text-right ${last?.balanceType==='Dr'?'text-orange-600':'text-green-700'}`}>{last?`₹${last.balance.toFixed(2)} ${last.balanceType}`:''}</td>
                        <td className="px-6 py-4 text-sm text-right text-orange-600">₹{totalDr.toFixed(2)}</td>
                        <td className="px-6 py-4 text-sm text-right text-green-700">₹{totalCr.toFixed(2)}</td>
                        <td></td>
                      </tr>
                    </tfoot>
                  )}
