...
# Let's copy the base prompt structure from extraction.py
def get_base_prompt(voucher_type):
    return f"""Extract {voucher_type} invoice data into this exact JSON schema:
...
for vtype in ['Purchase', 'Sales', 'Purchase'.upper(), 'Sales'.upper(), 'purchase', 'sales']:
    prompt = get_base_prompt(vtype).strip()
    h = hashlib.sha256(prompt.encode('utf-8')).hexdigest()
    print(f"voucher_type='{vtype}': hash={h}")
