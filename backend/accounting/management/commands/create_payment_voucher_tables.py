...
class Command(BaseCommand):
    help = 'Creates Payment Voucher tables in the database'

    def handle(self, *args, **kwargs):
        self.stdout.write("Checking if Payment Voucher tables exist...")
        with connection.cursor() as cursor:
            # 1. payment_vouchers
            cursor.execute("SHOW TABLES LIKE 'payment_vouchers'")
            if not cursor.fetchone():
                ...
                cursor.execute("""
                    CREATE TABLE payment_vouchers (
                        ...
                        FOREIGN KEY (pay_from_id) REFERENCES master_ledgers(id) ON DELETE RESTRICT
                    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
                """)
                self.stdout.write(self.style.SUCCESS("Table 'payment_vouchers' created."))
            else:
                self.stdout.write("Table 'payment_vouchers' already exists.")

            # 2. payment_voucher_items
            cursor.execute("SHOW TABLES LIKE 'payment_voucher_items'")
            if not cursor.fetchone():
                ...
                cursor.execute("""
                    CREATE TABLE payment_voucher_items (
                        ...
                        FOREIGN KEY (pay_to_ledger_id) REFERENCES master_ledgers(id) ON DELETE RESTRICT
                    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
                """)
                self.stdout.write(self.style.SUCCESS("Table 'payment_voucher_items' created."))
            else:
                self.stdout.write("Table 'payment_voucher_items' already exists.")
